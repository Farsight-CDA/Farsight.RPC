...
export default function ChainsPage() {
  ...
  return (
    <div class="space-y-6">
      <div>
        <h1 class="text-2xl text-white">Chains</h1>
        <p class="text-sm text-slate-400">Manage the chain identifiers used in the discovery API.</p>
      </div>
      ...
      <section class="rounded border border-white/10 bg-slate-900 p-5">
        <h2 class="text-lg text-white mb-4">New Chain</h2>
        <form class="flex gap-3" onSubmit={submit}>
          <input class="w-full max-w-sm rounded border border-white/10 bg-slate-950 px-3 py-2 text-sm" value={name()} onInput={(event) => setName(event.currentTarget.value)} placeholder="ethereum" />
          <button class="rounded bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-500" type="submit">Add Chain</button>
        </form>
      </section>
      <section class="rounded border border-white/10 bg-slate-900 p-5">
        <div class="overflow-x-auto">
          ...
          {!rowsQuery.data?.length && <div class="rounded border border-dashed border-white/10 px-4 py-6 text-sm text-slate-400">No chains created yet.</div>}
        </div>
      </section>
    </div>
  );
