...
export default function ChainsPage() {
  ...
  return (
    <div class="space-y-6">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div class="space-y-2"><h1 class="text-3xl font-semibold tracking-tight text-white">Chains</h1><p class="text-sm leading-6 text-slate-400">Manage the chain identifiers used in the discovery API.</p></div></div>
      ...
      <section class="space-y-5 rounded-[1.5rem] border border-white/10 bg-slate-900/80 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.22)] backdrop-blur">
        <h2 class="text-xl font-semibold tracking-tight text-white">New Chain</h2>
        <form class="flex flex-col gap-3 sm:flex-row" onSubmit={submit}>
          <input class="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-50 outline-none transition placeholder:text-slate-500 focus:border-blue-400/70 focus:ring-2 focus:ring-blue-500/30 sm:max-w-sm" value={name()} onInput={(event) => setName(event.currentTarget.value)} placeholder="ethereum" />
          <button class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-blue-50 shadow-lg shadow-blue-950/40 transition hover:bg-blue-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/50 disabled:pointer-events-none disabled:opacity-60 sm:self-start" type="submit">Add Chain</button>
        </form>
      </section>
      <section class="overflow-hidden rounded-[1.5rem] border border-white/10 bg-slate-900/80 shadow-[0_24px_80px_rgba(0,0,0,0.22)] backdrop-blur">
        <div class="overflow-x-auto">
          ...
          {!rowsQuery.data?.length && <div class="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-6 text-sm text-slate-400">No chains created yet.</div>}
        </div>
      </section>
    </div>
  );
